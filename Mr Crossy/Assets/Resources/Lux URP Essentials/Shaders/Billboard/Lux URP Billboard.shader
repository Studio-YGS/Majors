Shader "Lux URP/Billboard"
{
    Properties
    {
        ...
        [Header(Fog)]
        [Space(8)]
        //[Toggle(_APPLYLFOG)]         _ApplyFog("Enable Fog", Float) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //  Instance world position
                ...
                //  Recalulate viewDirWS
                    ...
                    output.tangentWS = billboardTangentWS;
                    output.bitangentWS = billboardBitangentWS;
        ...
        //  Shadow caster specific input
            ...
            VertexOutput ShadowPassVertex(VertexInput input)
            {
                ...
                float3 positionWS = TransformObjectToWorld(input.positionOS.xyz).xyz;
                    ...
                    float4 positionVS = mul(UNITY_MATRIX_MV, float4(0, 0, 0, 1.0));
                    positionVS.xyz += input.positionOS.xyz * float3(scale.xy, 1.0);
                    positionWS = mul(UNITY_MATRIX_I_V, positionVS).xyz;
                    positionWS -= lightDirectionWS * _ShadowOffset;
                #else
                    half3 viewDirWS = lightDirectionWS;
                    half3 billboardTangentWS = normalize(float3(-viewDirWS.z, 0, viewDirWS.x));
                //  Expand Billboard
                    float2 percent = input.texcoord.xy;
                    float3 billboardPos = (percent.x - 0.5) * billboardTangentWS;
                        ...
                        billboardPos.y += percent.y - 0.5;
                    #endif
                    positionWS = TransformObjectToWorld(float4(billboardPos, 1)).xyz;
                    positionWS -= lightDirectionWS * _ShadowOffset;
                #endif

                half3 normalWS = -lightDirectionWS;
